...
public class CarRaceConditionGameTest extends GameTest {
	...
	public void defineGame() {
		...
		concepts.add(new BadgeCollectionConceptBuilder().setName(
				"LegsToKangole").build());

		defineGameHelper(GAME, Arrays.asList("NoCarCounterTest",
				"PlayerCalendarTrip", "ClassWalkTrip", "PedibusKidTrip",
				"PedibusVolunteerTrip"), concepts);

		loadClasspathRules(GAME, Arrays.asList("rules/" + GAME + "/constants",
				"rules/" + GAME + "/calendartrips.drl", "rules/" + GAME
						+ "/classday.drl", "rules/" + GAME + "/classtrips.drl",
				"rules/" + GAME + "/legsbadges.drl", "rules/" + GAME
						+ "/pedibus.drl", "rules/" + GAME + "/weeklytasks.drl",
				"rules/" + GAME + "/challenges/ChallengeKmSettimanali.drl",
				"rules/" + GAME + "/challenges/ChallengeScuolaSenzAuto.drl",
				"rules/" + GAME + "/test/noCarDayTest.drl"));
